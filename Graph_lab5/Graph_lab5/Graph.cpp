...
std::pair<std::map<int, int>,int> Graph::TSP()
{
	std::map<int,int> path;
	std::map<int, int> minPath;
	std::map<int, bool> visited;
	int val = INT_MAX;
	int& minCost = val;
	back(path, visited, 0, 0, minCost, minPath);
	return std::make_pair(minPath, val);
}

void Graph::back(std::map<int, int> &path, std::map<int, bool> &visited, int pos, int cost, int& minCost, std::map<int, int> &minPath)
{
	if (pos == 0)
	{
		for(int v: parse_vertices())
		{
			path[pos] = v;
			visited[v] = true;
			cost = 0;
			back(path, visited, pos + 1, cost, minCost, minPath);
			visited[v] = false;
		}
	}
	else
	{
		for (int ov : parse_nout(path[pos - 1]))
		{
			path[pos] = ov;
			int edgeCost = get_cost(path[pos - 1], ov);
			cost += edgeCost;
			if (!visited[ov])
			{
				visited[ov] = true;
...
/*
bool isValid(std::map<int, int> path, int pos)
{
	for (int i = 0; i <= pos; i++)
		for (int j = i + 1; j <= pos; j++)
			if(path.find(i)!=path.end() && path.find(j) != path.end())
				if (path[i] == path[j] && j != path.size() - 1)
					return false;
	return true;
}*/