...
void printMenu()
{
	...
	std::cout << "18 - verifies if the graph is a DAG and performs a topological sorting of the activities.\n";
	std::cout << "19 - prints the earliest and the latest starting time for each activity and the total time of the project, as well as the critical activities.\n";
	std::cout << "0 - to exit.\n";
	std::cout << "**********************************************************\n";
}
...
void startUI()
{
	...
	while (1)
	{
		...
		{
			try {
				auto EaLSP = g.earliestAndLatestStartingPoint();
				std::cout << "Earliest starting points:\n";
				for (auto it = std::get<0>(EaLSP).begin(); it != std::get<0>(EaLSP).end(); it++)
				{
					for (int i : g.parse_nout(it->first))
					{
						if(g.get_cost(it->first,i)!=0)
							std::cout << it->first << "," << i << " ";
					}
					std::cout << it->second << "\n";
				}
				std::cout << "Latest starting points:\n";
				for (auto it = std::get<1>(EaLSP).begin(); it != std::get<1>(EaLSP).end(); it++)
				{
					for (int i : g.parse_nout(it->first))
					{
						if (g.get_cost(it->first, i) != 0)
				...
				for (auto it = std::get<2>(EaLSP).begin(); it != std::get<2>(EaLSP).end(); it++)
					std::cout << *it << "\n";
			}
			catch (const char* e) {
				std::cout << e << "\n";
